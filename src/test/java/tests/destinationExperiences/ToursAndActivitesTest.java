...
public class ToursAndActivitesTest extends BaseTest {
    ...
    public void testDestinationToursAndActivities(ITestContext context) {
        ...
        LinkedHashMap<String, Object> ratedActivityMap = response.jsonPath()
    ...
    public void testDesintationTourAndActivityByID(ITestContext context) {
        RestAssured
                ...
                .statusCode(200)
                .body("data.name",
                        log(logger, Matchers.equalTo(context.getAttribute("ratedActivityName")),
                                "Rated Activity Name by ID"))
                .body("data.geoCode",
                        log(logger, Matchers.allOf(Matchers.hasKey("latitude"), Matchers.hasKey("longitude")),
