...
public class FlightOffersTest extends BaseTest {
    ...
    public Map<String, Object> getQueryParams(String originLocationCode, String destinationLocationCode,
            String departureDate, String returnDate, Integer adults) {
    ...
    public void testValidSearchFlightOffers(String originLocationCode,
            String destinationLocationCode, String departureDate, String returnDate, Integer adults) {

        Map<String, Object> queryParams = getQueryParams(originLocationCode, destinationLocationCode, departureDate,
                returnDate, adults);
        Response response = RestAssured
                ...
                .body("data.numberOfBookableSeats.unique()",
                        log(logger, Matchers.everyItem(Matchers.greaterThanOrEqualTo(adults)), "numberOfBookableSeats"))
                .body("data.itineraries.segments.arrival.iataCode.flatten().unique()",
                        log(logger, Matchers.hasItem(destinationLocationCode),
                                "Atleast One Arrival IATACode matching with destination"))
                .body("data.itineraries.segments.departure.iataCode.flatten().unique()",
                        log(logger, Matchers.hasItem(originLocationCode),
                                "Atleast One Depature IATACode matching with Origin"))
                .extract().response();

        LocalDate depDate = LocalDate.parse(departureDate);
        Assertion.assertTrue(
                new HashSet<>(response.jsonPath().getList("data.itineraries.segments.departure.at.flatten()"))
                        .stream().map(ts -> LocalDateTime.parse((String) ts).toLocalDate())
    ...
    public Object[][] getvalidAirLineQuery() {
        ...
        LocalDate date = todaysDateTime.getHour() < 16 ? todaysDateTime.toLocalDate()
    ...
    public void testInvalidSearchFlightOffers(String originLocationCode,
            String destinationLocationCode, String departureDate, String returnDate, Integer adults, String errorTtile,
            String errorDetail) {
    ...
    public Object[][] getInvalidAirLineQuery() {
        ...
        LocalDate date = todaysDateTime.getHour() < 16 ? todaysDateTime.toLocalDate()
                : todaysDateTime.plusHours(10).toLocalDate();
        return new Object[][] {
                { "BLR", "BLR", date.plusDays(1).toString(), null, 2,
                        "INVALID DATA RECEIVED", "The multiple O/D overlap" },

                { "BLR", "NHH", date.plusDays(1).toString(), null, 3, "INVALID DATA RECEIVED",
                        "This location code is unknown" },

                { "BLR", "MAA", date.minusDays(2).toString(), null, 3, "INVALID DATE",
                ...
                { "MAA", "DEL", date.toString(), null, 0, "INVALID DATA RECEIVED", "adults must be between 1 and 9" },

                { "BLR", "DEL", date.toString(), date.minusDays(3).toString(),
